...
int main() {
  const struct device *status_led_dev =
      DEVICE_DT_GET(DT_NODELABEL(status_leds));
  if (!device_is_ready(status_led_dev)) {
    LOG_ERR("状态LED设备未就绪\n");
    return -1;
  }
  const auto led_api =
      static_cast<const status_leds_api *>(status_led_dev->api);
  ...
  while (true) {
    ...
    if (ret == 0) {
      ...
      if (ret == 0) {
        LOG_INF("Accel: x=%d.%06d, y=%d.%06d, z=%d.%06d", accel_val[0].val1,
                abs(accel_val[0].val2), accel_val[1].val1,
                abs(accel_val[1].val2), accel_val[2].val1,
                abs(accel_val[2].val2));
      } else {
    ...
    if (ret == 0) {
      ...
      if (ret == 0) {
        LOG_INF("Gyro: x=%d.%06d, y=%d.%06d, z=%d.%06d", gyro_val[0].val1,
                abs(gyro_val[0].val2), gyro_val[1].val1, abs(gyro_val[1].val2),
