...
class PIDController {
  ...
  static constexpr bool HasDeadband =
      (std::is_same_v<Features, WithDeadband> || ...);
  static constexpr bool HasIntegralLimit =
      (std::is_same_v<Features, WithIntegralLimit> || ...);
  static constexpr bool HasDerivativeOnMeasurement =
      (std::is_same_v<Features, WithDerivativeOnMeasurement> || ...);
  static constexpr bool HasDerivativeFilter =
      (std::is_same_v<Features, WithDerivativeFilter> || ...);
  static constexpr bool HasOutputFilter =
      (std::is_same_v<Features, WithOutputFilter> || ...);
  static constexpr bool HasOutputLimit =
      (std::is_same_v<Features, WithOutputLimit> || ...);

  static constexpr ValueType D_RC =
      static_cast<ValueType>(CONFIG_PID_Derivative_LPF_RC_M1K) /
      static_cast<ValueType>(1000.0);
  static constexpr ValueType O_RC =
      static_cast<ValueType>(CONFIG_PID_Output_LPF_RC_M1K) /
      static_cast<ValueType>(1000.0);
...
public:
  explicit PIDController(ValueType Kp, ValueType Ki, ValueType Kd,
      ...
      : MaxOutput(MaxOutput.value_or(std::numeric_limits<ValueType>::max())),
        ...
        IntegralLimit(
            IntegralLimit.value_or(std::numeric_limits<ValueType>::max())),
        Kp(Kp), Ki(Ki), Kd(Kd), deltaT() {}
